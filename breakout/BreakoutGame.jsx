...
const BreakoutGame = () => {
  ...
  // Uses createBricksData from gameLogic.js and sets pinball feature states
  const createBricks = useCallback((level, enemy) => {
    const { bricks, bumpers, portals, spawners } = createBricksData(level, enemy);
  ...
  const createBall = (level = 1, enemyIndex = 0) => {
    ...
    const currentPaddle = paddleRef.current;
    const paddleX = currentPaddle ? currentPaddle.x : null;
    const paddleWidth = currentPaddle ? currentPaddle.width : PADDLE_WIDTH;
    return createBallData(level, enemyIndex, paddleX, paddleWidth);
  };
  ...
  const spawnEnemy = useCallback(() => {
    ...
    return createEnemyData(difficulty, enemyId);
